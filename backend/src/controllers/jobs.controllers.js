...
export const createCurriculum = async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res
        .status(400)
        .json({ message: "La foto de perfil es obligatoria." });
    }
    ...
    if (req.body.profesion === "otros" && req.body.otherProfession) {
      const newProfession = new Profession({
        profession: req.body.otherProfession,
      });
...
export const getCurriculumById = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error al obtener el curriculum", error });
  }
};
  ...
  try {
    ...
    const curriculumsWithBase64Image = curriculums.map((curriculum) => {
      ...
      if (curriculumObject.profilePhoto && curriculumObject.profilePhoto.data) {
        curriculumObject.profilePhoto.data =
          curriculumObject.profilePhoto.data.toString("base64");
      }
    ...
    res.status(500).json({
      message: "Error al obtener los curriculums",
      error: error.message,
    });
  }
};
...
export const updateCurriculum = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error al actualizar el curriculum", error });
  }
};
...
export const deleteCurriculum = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error al eliminar el curriculum", error });
  }
};
